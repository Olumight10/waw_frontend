...
export default function QuickLoginModal({ uniqueCode, onSuccess, onClose }: QuickLoginModalProps) {
  ...
  return (
    ...
    >
      <div 
        className="bg-white rounded-xl shadow-2xl max-w-md w-full p-8 border-t-8 border-church-gold relative"
        ...
        {view === 'login' ? (
          <>
            ...
            <div className="flex gap-4">
              <button onClick={onClose} className="w-1/2 bg-gray-200 py-3 rounded-lg text-sm font-bold uppercase">Cancel</button>
              <button onClick={handleLogin} className="w-1/2 bg-church-purple text-white py-3 rounded-lg font-bold text-sm uppercase">Login</button>
            </div>
          </>
        ) : (
